...
#  文本转HTML
def text2html(text):
    lines = map(lambda s:'<p>%s</p>' % s.replace('&', '&amp').replace('<', '&lt').replace('>', '&gt'),
...
async def index(*, page='1'):
    ...
    p = Page(num, page_index)   #page_index?？是啥？
    if num == 0:
        blogs = []
    else:
...
async def api_blogs(*, page='1'):
    page_index = get_page_index('count(id)')
    num = await Blog.findNumber('count(id)')
    p = Page(num, page_index)
    if num == 0:
        return dict(page=p, blogs=())
    blogs = await Blog.findAll(orderBy='created_by desc', limit=(p.offset, p.limit))
